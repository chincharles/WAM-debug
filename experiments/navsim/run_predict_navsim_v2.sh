...
MASTER_PORT="${MASTER_PORT:-27892}"
CKPT="${CKPT:-./weights/checkpoint.pt}"
EXP_NAME="${EXP_NAME:-simwam_navsim_v2}"
SCENE_FILTER="${SCENE_FILTER:-./navsim_v2/navsim/planning/script/config/common/train_test_split/scene_filter/navtest.yaml}"
PRED_DIR="${PRED_DIR:-./evaluate_results_v2/navtest/${EXP_NAME}/pred_actions}"

echo "===== Predicting NAVSIM v2 navtest: ${CKPT} (${EXP_NAME}) ====="
torchrun \
  --nproc_per_node "${NPROC_PER_NODE}" \
  --nnodes 1 \
  --master-port "${MASTER_PORT}" \
  experiments/navsim/predict_navsim_v2.py \

...
class WanVideoDiT(torch.nn.Module):
    ...
    def pre_dit(
        ...
        if self.seperated_timestep and fuse_vae_embedding_in_latents:
            ...
            token_timesteps[:, 0, :] = 0    # first frame use timestep 0, which will be the same as the timestep used for vae embedding if fuse_vae_embedding_in_latents is True, so that the model can learn to align the first latent frame with the vae embedding when timestep=0

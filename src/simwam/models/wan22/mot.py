...
class MoT(nn.Module):
    ...
    def forward_video_only(
        ...
        """Run the video expert with video-only self-attention and return the updated tokens.

        Same block loop as `prefill_video_cache` but returns the post-block video tokens
        (for `post_dit -> velocity`) instead of the K/V cache, and supports gradients
        (used by video GRPO's `video_velocity`).
        """
        if "video" not in self.mixtures:
            raise ValueError("MoT requires `video` expert for `forward_video_only`.")
        if video_attention_mask.ndim != 2 or video_attention_mask.shape[0] != video_attention_mask.shape[1]:
            raise ValueError(f"`video_attention_mask` must be square 2D, got {tuple(video_attention_mask.shape)}")
        if video_attention_mask.shape[0] != video_tokens.shape[1]:
            raise ValueError(
                f"`video_attention_mask` seq len {video_attention_mask.shape[0]} != tokens {video_tokens.shape[1]}"
            )

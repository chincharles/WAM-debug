...
class SimWAM(torch.nn.Module):
    ...
    def __init__(
        ...
        if action_prediction_type not in ("velocity", "sample"):
            raise ValueError(
                f"`action_prediction_type` must be 'velocity' or 'sample', got {action_prediction_type!r}"
            )
        if not (0.0 < action_sigma_clamp_min <= 1.0):
            raise ValueError(
    ...
    def _action_sigma(self, timestep: torch.Tensor) -> torch.Tensor:
        """σ = t/T (SimWAM noising convention), clamped to [sigma_min, 1]."""
        sigma = timestep.to(dtype=torch.float32) / float(
            self.train_action_scheduler.num_train_timesteps
        )
        return sigma.clamp(min=self.action_sigma_clamp_min, max=1.0)

    def _action_x_to_v(
        self, pred_x: torch.Tensor, noisy_action: torch.Tensor, timestep: torch.Tensor
    ) -> torch.Tensor:
        """Convert a clean-sample prediction x̂ into a flow-matching velocity v̂.

        With x_σ = (1-σ)·x_data + σ·noise, v* = noise - x_data, and x̂ = x̂_data:
            v̂ = (x_σ - x̂) / σ
        σ is clamped via `_action_sigma` for numerical stability near σ→0.
        """
        sigma = self._action_sigma(timestep).to(device=noisy_action.device)
        sigma = sigma.view(-1, *([1] * (noisy_action.ndim - 1)))
        return (noisy_action - pred_x) / sigma.to(dtype=noisy_action.dtype)
    ...
    def build_inputs(self, sample, tiled: bool = False):
        ...
        if "context" not in sample or "context_mask" not in sample:
            raise ValueError(
                "SimWAM training requires `sample['context']` and `sample['context_mask']`."
            )
        ...
        if "action" not in sample:
            raise ValueError("`sample['action']` is required for SimWAM training.")
    ...
    def training_loss(self, sample, tiled: bool = False):
        ...
        if self.action_prediction_type == "sample":
            # x-prediction: network targets the clean action sample.
        ...
        if self.action_prediction_type == "sample":
            # v-loss equivalent: ‖v̂ - v*‖² = ‖x̂ - x_data‖² / σ², with σ=(1-t) clamped.
            sigma = self._action_sigma(timestep_action).to(action_loss_token.device)  # [B]
            action_loss_token = action_loss_token / (sigma * sigma).unsqueeze(1)
        if action_is_pad is not None:

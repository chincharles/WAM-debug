    ...
    def from_camera_dict(
        ...
        for camera_name in camera_dict.keys():
            ...
            if camera_identifier in sensor_names:
                ...
                # Use a context manager so the file handle is released immediately; the bare
                # `np.array(Image.open(path))` leaves the fp open until GC, which can exhaust
                # file descriptors over a long eval (OSError: [Errno 24] Too many open files).
                with Image.open(image_path) as _pil_image:
                    image_array = np.array(_pil_image)
